...
<form method="POST" action="{{ route('admin.settings.update') }}">
    ...
    <div class="card" style="margin-bottom:20px">
        ...
        <div style="padding:20px">
            <div>
                <label style="display:block;margin-bottom:6px;font-size:13px;color:var(--text2)">Referral Bonus Amount (Rs.)</label>
                <input type="number" name="referral_bonus"
                       value="{{ old('referral_bonus', $settings['referral_bonus']) }}"
                       class="form-input" min="0" step="1" style="max-width:200px">
            </div>
        </div>
    </div>
        ...
        <div style="padding:20px">
            <div style="display:grid;grid-template-columns:1fr 1fr;gap:16px">
                ...
                <div style="display:flex;align-items:center;gap:10px;margin-top:24px">
                    ...
                    <label for="maintenance_mode" style="font-size:14px">🔧 Maintenance Mode</label>
                </div>
            </div>
        </div>
    </div>
...
<div class="card" style="margin-top:24px">
    ...
    <div style="padding:20px">
        ...
        <form method="POST" action="{{ route('admin.settings.change-password') }}">
            ...
            <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:16px">
                <div>
                    <label style="display:block;margin-bottom:6px;font-size:13px;color:var(--text2)">Current Password</label>
                    <input type="password" name="current_password" class="form-input" placeholder="Enter current password">
                </div>
                <div>
                    <label style="display:block;margin-bottom:6px;font-size:13px;color:var(--text2)">New Password</label>
                    <input type="password" name="new_password" class="form-input" placeholder="Min 6 characters">
                </div>
                <div>
                    <label style="display:block;margin-bottom:6px;font-size:13px;color:var(--text2)">Confirm New Password</label>
                    <input type="password" name="new_password_confirmation" class="form-input" placeholder="Repeat new password">
                </div>
            </div>
